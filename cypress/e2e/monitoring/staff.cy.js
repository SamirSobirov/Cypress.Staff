describe('Staff Management Flow', () => {
  ...
  it('Полный цикл: Авторизация -> Добавление -> Изменение -> Удаление', () => {
   ...
   // =========================================================
    ...
    cy.wait(1000); // Даем окну время полностью отрисоваться

    // 2. ИСПРАВЛЕНИЕ: Клик по кнопке в модальном окне
    // Используем точный класс со скриншота 880601
    cy.get('.app-confirm-modal__button--accept', { timeout: 10000 })
      .should('be.visible')
      .click({ force: true }); // force: true поможет, если кнопка перекрыта прозрачным слоем анимации

    cy.log('✅ Удаление подтверждено');

    // 3. Проверка исчезновения
    // Ждем, пока надпись "Sobiros Samir" исчезнет из таблицы
